...
RUN apt-get update \
 && apt-get install --yes --no-install-recommends \
      ...
      wget \
      zlib1g-dev \
    # Trex needs libzmq5, which we don't have in the stateless apt repository yet. TODO add it into the next version...
...
# bunch of build errors which are assumed to be due to an incompatability between DPDK 19.11 and rdma-core 25.0.
RUN git clone --branch v19.11 http://dpdk.org/git/dpdk \
 && cd dpdk \
...
# need to do this.
RUN git clone --branch v2.71 https://github.com/cisco-system-traffic-generator/trex-core.git \
 ...
 && git clone --branch v19.05 http://dpdk.org/git/dpdk dpdk1905 \
    # Make a directory called dpdk_libs which contains only the dpdk/libs directories that Trex uses.
 && mkdir dpdk_libs \
 && for DPDK_LIB in $(find dpdk1905/lib/* -maxdepth 1 -type d -name "lib*"); do \
 ...
 && rm trex-core/src/dpdk/lib/librte_net/rte_ip.h dpdk_libs/librte_net/rte_ip.h \
    ...
    # DPDK 19.11 source.
 && diff -r --unified dpdk_libs trex-core/src/dpdk/lib > trex.patch || true \
 && rm -rf dpdk1905 \
...
# RUN cd tmp \
# 	 && wget https://trex-tgn.cisco.com/trex/release/v2.71.tar.gz \
# 	 && tar -xf v2.71.tar.gz \
# 	 && rm v2.71.tar.gz \
# 	 && mv v2.71 trex

# Copy our Trex configuration files into the image.
COPY traffic-configurations/mlx_cfg.yaml /etc/trex_cfg.yaml
COPY traffic-patterns/dns_customer_ip.yaml /tmp/dns_customer_ip.yaml
